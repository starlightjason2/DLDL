...
* Creates a 2x2 subplot visualization with:
  ...
  - Validation F1 Score
* Saves plot to `GRAPH_DIR/{JOB_ID}_training_log_plot.png` (or `PROG_DIR` if `GRAPH_DIR` not set)

**Command-line Parameters:**

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `--csv` | str | `PROG_DIR/{JOB_ID}_training_log.csv` | Path to the training log CSV file. If not specified, uses the default path based on `PROG_DIR` and `JOB_ID` constants. |
| `--output` | str | `GRAPH_DIR/{JOB_ID}_training_log_plot.png` | Path where the plot image will be saved. If not specified and `--show` is not used, defaults to `GRAPH_DIR/{JOB_ID}_training_log_plot.png`. If `--show` is used without `--output`, the plot is only displayed and not saved. |
